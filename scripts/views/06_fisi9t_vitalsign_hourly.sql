...
DROP VIEW IF EXISTS mimiciv_derived.fisi9t_vitalsign_hourly CASCADE;
...
base AS (
  SELECT
    v.subject_id, v.stay_id, v.charttime,
    v.heart_rate, v.sbp, v.dbp, v.mbp,
    v.sbp_ni, v.dbp_ni, v.mbp_ni,
    v.resp_rate, v.temperature, v.temperature_site, v.spo2, v.glucose
  FROM mimiciv_derived.vitalsign v
  JOIN cohort c ON c.subject_id = v.subject_id AND c.stay_id = v.stay_id
),
marked AS (
  SELECT
    b.*,
...
clustered AS (
  SELECT m.*, sum(m.is_new_cluster) OVER (PARTITION BY m.stay_id ORDER BY m.charttime) AS cluster_id
  FROM marked m
),
cluster_agg AS (
  SELECT
    c.subject_id, c.stay_id, c.cluster_id,
...
cluster_to_hour AS (
  SELECT
    ca.subject_id, ca.stay_id,
    date_trunc('hour', ca.cluster_time + interval '30 minutes') AS hour_ts,
    ca.heart_rate, ca.sbp, ca.dbp, ca.mbp,
    ca.sbp_ni, ca.dbp_ni, ca.mbp_ni,
    ca.resp_rate, ca.temperature, ca.temperature_site, ca.spo2, ca.glucose,
    ca.cluster_time
  FROM cluster_agg ca
),
hourly_obs AS (
  SELECT
    cth.subject_id, cth.stay_id, cth.hour_ts,
...
SELECT
  ...
  h.heart_rate, h.sbp, h.dbp, h.mbp,
  h.sbp_ni, h.dbp_ni, h.mbp_ni,
  h.resp_rate, h.temperature, h.temperature_site, h.spo2, h.glucose
FROM hour_grid g
LEFT JOIN hourly_obs h ON h.stay_id = g.stay_id AND h.hour_ts = g.hour_ts;

CREATE INDEX ON mimiciv_derived.fisi9t_vitalsign_hourly (stay_id, charttime_hour);
CREATE INDEX ON mimiciv_derived.fisi9t_vitalsign_hourly (subject_id);
